...
the ExampleProject repo into this standalone repo (`tyleradams2002/studio-stud`), cleaning ExampleProject
completely, and setting the project up to be hosted on GitHub + GitHub Pages for one-command install
into ExampleProject and any other project.

## 1. Goals (locked with Tyler)

1. Move **absolutely everything** Studio Stud from ExampleProject into this repo. This becomes the
   project's permanent home.
2. **Strip ExampleProject entirely** of Studio Stud. It re-acquires the tool only via the installer.
...
6. **Clean copy** (git history is NOT preserved; history remains in ExampleProject).

## 2. Source-of-truth inventory (verified on disk in ExampleProject)
...
- Consumer policy: `.studio-stud/policy.json` (kept as a reference template, not as ExampleProject config)
...
- `.cursor/rules/roblox-example-project.mdc` — remove "Studio Stud verify before depending" lines
...
- [x] **Step 5 — Strip ExampleProject** per section 7. Group A deleted (`tools/studio_stud/`, root
      ...
      Studio Stud references removed from `repo-navigation.mdc`, `roblox-example-project.mdc`,
      ...
      **Deviation:** ExampleProject's real `.studio-stud/policy.json` (place IDs + owned services) was
      KEPT, not deleted — the installer preserves an existing policy, so wiping it would have lost
      working config.
- [x] **Step 6 — Reinstall into ExampleProject** via the published one-liner with `-WithCursorRule`:
...
- **Git history not preserved** (Tyler's choice). History stays recoverable in ExampleProject.
