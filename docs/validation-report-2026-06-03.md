...
**Repo under test:** `C:\Users\tyler\OneDrive\Documents\GitHub\FishersLifeTESTING` (Rojo project name `FishersLife`)
**Place:** `109595751023912`, ~**38,737** instances · **Daemon:** `http://127.0.0.1:31878`, protocol 1
...
- `%LOCALAPPDATA%\StudioStud\config.json` after a full session: `installRoot`,`pluginsDir`,`channel`,`versions.*` all **empty strings**; `pathShimInstalled:false` (a shim exists); `lastChannelSequence:{}`; `repos` lists **only** `…\GitHub\studio-stud` (the tool's own repo), **not** `FishersLifeTESTING`.
- Yet capture/live/query all resolved `FishersLife` correctly and the serve banner said **"Registry: 2 repo(s)"** — so the working binding came from a **different** registry than `config.json`, which was never populated/repaired on connect.
...
- **Evidence:**
  - `Get-Command studio-stud -All` returns **5** entries: `C:\WINDOWS\system32\studio-stud.ps1` (+`.cmd`) ← **wins precedence**; `…\GitHub\FishersLife\studio-stud.ps1` (+`.cmd`) ← a **sibling** repo (not `FishersLifeTESTING`); `…\AppData\Local\StudioStud\bin\studio-stud.exe`.
...
- **Repro:** `"allowedPlaceIds": ["109595751023912"]` (string, as emitted by `status`/`query`/widget and implied by the runbook).
- **Actual:** `Error: invalid type: string "109595751023912", expected i64 at line 2 column 39` — never names `allowedPlaceIds`. Integer form `[109595751023912]` works, after which the engine is correct (see "Validated solid").
...
studio-stud analyze 109595751023912 --report context --limit 10
studio-stud query  109595751023912 --find <Name> [--count-only]

# repo-root bug (F-B): relative fails, absolute works
studio-stud project check --repo-root .                  # FAILS (os error 2)
studio-stud project check --repo-root C:\...\FishersLifeTESTING   # ok:true

# policy engine (correct once placeId is an INTEGER — F-K)
studio-stud policy explain --path src/Shared/X.luau --place 109595751023912 --repo-root <ABS>
...
- `%LOCALAPPDATA%\StudioStud\FishersLife\places\109595751023912\syncs.db` — **~287 MB** (F-H)
