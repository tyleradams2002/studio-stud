...
Studio Stud is the AI-first live Roblox Studio inspection path for Example Project. It captures as much read-only Studio metadata as practical, stores the heavy data locally, indexes it into SQLite, and exposes compact JSON query surfaces for agents.
...
Daemon connection count reference (measured June 2026, ExamplePlaceA):
...
.\studio-stud query 100000000000001 --find Trader --limit 10
.\studio-stud query 100000000000001 --name BoatSpawnPoints
.\studio-stud query 100000000000001 --path Workspace/BoatSpawnPoints
```

Use scoped relative results for hierarchy work:

```powershell
.\studio-stud query 100000000000001 --under Workspace/BoatSpawnPoints --class Part --limit 10
.\studio-stud query 100000000000001 --tree Workspace/BoatSpawnPoints --depth 1 --limit-siblings 25
```

Use explicit property selection for details:

```powershell
.\studio-stud query 100000000000001 --detail Workspace/BoatSpawnPoints --props Position,Size
.\studio-stud query 100000000000001 --detail MeshPart:000123 --props MeshId,TextureID,RenderFidelity,CollisionFidelity
.\studio-stud query 100000000000001 --detail MeshPart:000123 --all
```
...
```powershell
.\studio-stud query 100000000000001 --bulk '{"queries":[{"key":"boatSpawns","path":"Workspace/BoatSpawnPoints"},{"key":"spawnTree","tree":"Workspace/BoatSpawnPoints","depth":1,"limitSiblings":10},{"key":"dockProps","detail":"Workspace/Dock","props":["Position","Size"]}]}'
.\studio-stud query 100000000000001 --bulk @bulk.json
```
...
```powershell
.\studio-stud query 100000000000001 --detail MeshPart:000123
```
...
.\studio-stud query 100000000000001 --find Trader --limit 10 --markdown
```
...
**Ownership rule (Rojo v7):** `$path` nodes default `ignoreUnknownInstances: false` unless explicit; pure tree nodes (no `$path`) default `true`. ExampleProject service roots set `ignore:true`, but directory-projected folders (e.g. `ServerScriptService/Core`) default `false` — Studio-only children there surface as `extraInStudio`, not under the service root.
