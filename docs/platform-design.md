...
ExampleProject-internal planning notes that were not migrated into this standalone repo; the staged
...
- Boat plugin: separate `plugin:CreateToolbar("Example Project")` + dock widget. Audit + Fix dashboard
...
flowchart TB
  ...
  subgraph project [Project layer - Example Project]
...
Answering "should we only watch Luau?": for Example Project, the synced source is almost entirely Luau
...
Example Project, leave it off everywhere else (Decision D2: one plugin, one update target).
...
Each place (e.g., Example Place A, Example Place B) keeps its own single live DB — never a shared one.
...
command-bar scripts. The tab is enabled in settings for Example Project and off elsewhere (portable).
...
`tools/plugin/BoatConfigurator.plugin.lua`, the "Example Project" toolbar, and the boat-specific token
...
- Exit gate: bounded diff JSON against Example Project; correct repo-owned vs Studio-owned reporting.
...
{
  "version": 1,
  "allowedPlaceIds": [100000000000001],
