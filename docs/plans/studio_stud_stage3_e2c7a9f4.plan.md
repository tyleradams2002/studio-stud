...
- **CLI today** (`cli.rs`): `Cli{command}`, `Commands` enum with `Status/Doctor/Ingest/Analyze/Query/Capture/
  Serve/Daemon/Bench/LiveDelta/LiveVerify/LiveDump`. `CommonArgs{project_key(default "FishersLife"),
...
- **Util** (`util.rs`): `DEFAULT_HOST="127.0.0.1"`, `DEFAULT_PORT=31878`, `DEFAULT_PROJECT_KEY="FishersLife"`,
...
B2. CLI subcommands (NOT hidden — these are user-facing per §5.3/Appendix B):
   - `studio-stud policy init [--repo-root <p>] [--force]` — write a default `.studio-stud/policy.json` if
     absent (refuse overwrite without `--force`). Default: `version:1`, `allowedPlaceIds:[139581542512435,
     87774153727073]` (Mike's Resort, Kelley's — from the studio-stud rule), a MINIMAL `allowedWritePaths`
...
F1. Run `studio-stud policy init` at the FishersLife repo root and COMMIT the resulting `.studio-stud/
