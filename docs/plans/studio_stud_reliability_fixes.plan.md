...
All paths are relative to the workspace root (the FishersLife repo root). Tool
...
- **`active_place` staleness on island switch.** `resolve_place` falls back to the
  `active_place` file; immediately after switching islands (e.g. MikesResort →
  KelleysIsland) a query without an explicit `<PLACE>` may briefly resolve the previous
