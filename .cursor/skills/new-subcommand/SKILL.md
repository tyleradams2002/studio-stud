...
2. **Handler module.** Create `src/commands/<name>.rs` (match repo layout) with `run(args) -> Result<Output>`, where `Output` is a dedicated serde struct — the stable JSON contract for this command.
